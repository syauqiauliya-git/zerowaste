...
import {
  getAllSPPGAssignments,
  createSPPGAssignment,
  updateSPPGAssignment,
  deleteSPPGAssignment,
  SPPGSchoolAssignment,
} from "@/lib/assignments";
import { fetchAllSPPG, SPPG } from "@/lib/sppg";
import { fetchSchools, School } from "@/lib/school";

export default function SPPGAssignments() {
  const [assignments, setAssignments] = useState<SPPGSchoolAssignment[]>([]);
  ...
  const [showSPPGPicker, setShowSPPGPicker] = useState(false);
  const [showSchoolPicker, setShowSchoolPicker] = useState(false);
  const [tempSPPGId, setTempSPPGId] = useState("");
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [assignmentsRes, sppgRes, schoolsRes] = await Promise.all([
        getAllSPPGAssignments(),
        fetchAllSPPG(),
        fetchSchools(),
      ]);
  ...
  const handleAddAssignment = async () => {
    if (!formData.sppg_id || !formData.school_id) {
      setErrorMessage("Please select both SPPG and school");
      setShowErrorModal(true);
      return;
    }
    ...
    try {
      if (editingId) {
        await updateSPPGAssignment(editingId, formData);
        setSuccessMessage("Assignment updated successfully");
      } else {
        await createSPPGAssignment(formData);
        setSuccessMessage("Assignment added successfully");
      }
      setFormData({ sppg_id: "", school_id: "", is_active: true });
      setShowForm(false);
      setEditingId(null);
  ...
  const handleEdit = (assignment: SPPGSchoolAssignment) => {
    setFormData({
      sppg_id: assignment.sppg_id._id,
      school_id: assignment.school_id._id,
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await deleteSPPGAssignment(deleteId);
  ...
  const renderItem = ({ item }: { item: SPPGSchoolAssignment }) => (
    <View style={styles.card}>
      <View style={{ flex: 1 }}>
        <Text style={styles.cardTitle}>{item.sppg_id.name}</Text>
        <Text style={styles.cardSubtitle}>
          {item.school_id.school_name} • {item.school_id.address}
        </Text>
  ...
  const filteredAssignments = assignments.filter((assignment) => {
    ...
    return (
      assignment.sppg_id.name.toLowerCase().includes(query) ||
      (assignment.school_id.school_name || "").toLowerCase().includes(query)
    );
  });
  ...
  return (
    <View style={styles.container}>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.form}>
              <View style={styles.formGroup}>
                <Text style={styles.label}>SPPG Organization</Text>
                <View style={styles.pickerContainer}>
                  <TouchableOpacity
                    style={styles.pickerButton}
                    onPress={() => {
                      setTempSPPGId(formData.sppg_id);
                      setShowSPPGPicker(true);
                    }}
                  >
                    <Text style={styles.pickerButtonText}>
                      {formData.sppg_id
                        ? sppgList.find((s) => s._id === formData.sppg_id)
                            ?.name || "Select SPPG"
                        : "Select SPPG"}
                    </Text>
              ...
              <View style={styles.formGroup}>
                ...
                <View style={styles.pickerContainer}>
                  <TouchableOpacity
                    style={styles.pickerButton}
                    onPress={() => {
                      setTempSchoolId(formData.school_id);
                  ...
                  >
                    <Text style={styles.pickerButtonText}>
                      {formData.school_id
                        ? (() => {
                            const sch = schools.find((s) => s._id === formData.school_id);
                            return sch ? `${sch.school_name} - ${sch.address || "N/A"}` : "Select school";
                          })()
                        : "Select school"}
                    </Text>
                    <MaterialIcons name="arrow-drop-down" size={24} color="#666" />
      ...
      {/* SPPG Picker Modal */}
      <Modal
        visible={showSPPGPicker}
        animationType="fade"
        transparent
        onRequestClose={() => setShowSPPGPicker(false)}
      >
        <View style={styles.pickerModalBackdrop}>
          <View style={styles.pickerModalContent}>
            <View style={styles.pickerModalHeader}>
              <Text style={styles.pickerModalTitle}>Select SPPG</Text>
              <TouchableOpacity onPress={() => setShowSPPGPicker(false)}>
                <MaterialIcons name="close" size={24} color="#666" />
              </TouchableOpacity>
            </View>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[
                    styles.pickerItem,
                    tempSPPGId === item._id && styles.pickerItemSelected,
                  ]}
                  onPress={() => setTempSPPGId(item._id)}
                >
                  <Text
                    style={[
                      styles.pickerItemText,
                      tempSPPGId === item._id && styles.pickerItemTextSelected,
                    ]}
                  >
                    {item.name}
                  </Text>
            ...
            <View style={styles.pickerModalButtons}>
              <TouchableOpacity
                ...
                onPress={() => setShowSPPGPicker(false)}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setFormData({ ...formData, sppg_id: tempSPPGId });
                  setShowSPPGPicker(false);
                }}
              >
          ...
          <View style={styles.pickerModalContent}>
            ...
            <View style={styles.pickerModalButtons}>
              ...
              <TouchableOpacity
                style={[styles.pickerButton2, styles.pickerOkButton]}
                onPress={() => {
                  setFormData({ ...formData, school_id: tempSchoolId });
